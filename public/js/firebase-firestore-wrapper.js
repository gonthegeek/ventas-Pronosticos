...
export let getFirestore, collection, addDoc, query, onSnapshot, orderBy, doc, writeBatch, Timestamp, where, deleteDoc, getDocs, limit, startAfter;
// Preload immediately in browser; in node tests consumers will mock before usage
load().then(mod => {
  ({ getFirestore, collection, addDoc, query, onSnapshot, orderBy, doc, writeBatch, Timestamp, where, deleteDoc, getDocs, limit, startAfter } = mod);
}).catch(()=>{});
