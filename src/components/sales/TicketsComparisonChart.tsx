...
interface TicketsComparisonChartProps {
  data: Array<{
    date: string;
    displayName: string;
    totalTickets: number;
...
const TicketsComparisonChart: React.FC<TicketsComparisonChartProps> = ({
  data,
  selectedMachines,
  mode = 'line',
}) => {
        ...
        <div className="bg-white p-3 border border-gray-300 rounded-lg shadow-lg">
          ...
          {payload.map((entry: any, index: number) => (
            <p key={index} className="text-sm" style={{ color: entry.color }}>
              {entry.name}: <strong>{entry.value.toLocaleString('es-MX')} boletos</strong>
            </p>
          ))}
        </div>
      );
...
export default TicketsComparisonChart;
